...
public sealed class BackupsViewModel : ObservableObject
{
    ...
    public BackupsViewModel(IBackupService backups, ISettingsService settings, IUiErrorHandler errors)
    {
        _backups = backups; _settings = settings; _errors = errors;
    ...
    private void SetError(Exception exception) => Message = _errors.Handle(exception, "Backup operation");
}
